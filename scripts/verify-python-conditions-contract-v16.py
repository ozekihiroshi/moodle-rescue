#!/usr/bin/env python3
"""Verify the Lesson 1.5 concept map and bilingual Notebook metadata."""
...
MAP = ROOT / "sample-content/introduction-to-python/localization/lesson-1-5-concept-map-v1.json"
...
def main() -> None:
    ...
    assert [item["question"] for item in concepts] == [f"L15R-{i:02d}" for i in range(1, 11)]
    ...
    for language, relative in data["notebooks"].items():
        ...
        assert metadata["lesson"] == "1.5"
        assert metadata["language"] == language
        assert metadata["revision"] == 16
    ...
    print(json.dumps({"verified": True, "concepts": 10, "questions": 10, "languages": 2, "revision": 16}, indent=2))
