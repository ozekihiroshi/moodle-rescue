...
artifact="$dist_dir/python-for-data-foundations-ai-era-1.0.0.mbz"
...
printf '%s\n' "$metadata" | grep -q '<original_course_shortname>PYAI-INTRO</original_course_shortname>'
printf '%s\n' "$metadata" | grep -q '<moodle_release>5.2.2 (Build: 20260810)</moodle_release>'

echo 'Python sample distribution verified.'
