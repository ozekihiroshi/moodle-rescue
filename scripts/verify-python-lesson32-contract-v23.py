#!/usr/bin/env python3
"""Verify the Lesson 3.2 concept map and bilingual Notebook metadata."""
...
MAP = ROOT / "sample-content/introduction-to-python/localization/lesson-3-2-concept-map-v1.json"
...
def main() -> None:
    ...
    assert [item["question"] for item in concepts] == [f"L32R-{i:02d}" for i in range(1, 11)]
    ...
    required = ("find_course_data", ".loc[", ".iloc[", ".isin(", ".between(", ".notna()", ".isna()", " & ", " | ", "~", ".sum()", "sort_values(")
    for language, relative in data["notebooks"].items():
        document = json.loads((ROOT / relative).read_text(encoding="utf-8"))
        metadata = document["metadata"]["pyai"]
        assert metadata["lesson"] == "3.2"
        assert metadata["language"] == language
        assert metadata["revision"] == 23
    ...
    print(json.dumps({"verified": True, "concepts": 10, "questions": 10, "languages": 2, "revision": 23}, indent=2))
