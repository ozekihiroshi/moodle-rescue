...
tools_root=/tmp/ipa-ap-preview-tools
...
compose exec -T moodle mkdir -p "$tools_root"
compose cp scripts/update-ipa-ap-source-study-course.php "moodle:$tools_root/"
compose cp scripts/update-ipa-ap-preview-course.php "moodle:$tools_root/"
cleanup() {
    compose exec -T moodle rm -rf "$container_root" "$tools_root"
}
trap cleanup EXIT INT TERM

compose exec -T \
    -e IPA_AP_CONTENT_ROOT="$container_root" \
    moodle runuser -u www-data -- php "$tools_root/update-ipa-ap-preview-course.php"
