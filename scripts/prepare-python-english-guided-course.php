...
$shortname = 'PYAI-INTRO-EN-DUAL-GUIDED';
if ($DB->record_exists('course', ['shortname' => $shortname])) { throw new RuntimeException('Already exists; do not overwrite'); }
$source = $DB->get_record('course', ['shortname' => 'PYAI-INTRO-EN-DUAL-PATH'], '*', MUST_EXIST);
$result = core_course_external::duplicate_course($source->id,
    '[Draft] Python Foundations — Teacher-guided Markdown edition', $shortname, $source->category, 0,
    [['name' => 'users', 'value' => 0]]);
...
$path = make_temp_directory('python-english-guided-full') . '/mapping.json';
