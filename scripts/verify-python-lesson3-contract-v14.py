#!/usr/bin/env python3
"""Verify the Lesson 1.3 concept map and Notebook metadata."""
...
MAP = ROOT / "sample-content/introduction-to-python/localization/lesson-1-3-concept-map-v1.json"
...
def main() -> None:
    ...
    assert [item["question"] for item in concepts] == [f"L13R-{i:02d}" for i in range(1, 11)]
    ...
    for language, relative in data["notebooks"].items():
        ...
        assert metadata["revision"] == 14
        assert metadata["concepts"] == expected
        assert document["nbformat"] == 4
    print(json.dumps({"verified": True, "concepts": 10, "questions": 10, "languages": 2, "revision": 14}, indent=2))
